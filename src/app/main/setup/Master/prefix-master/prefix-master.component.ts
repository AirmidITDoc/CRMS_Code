            ...
            if (!this._PrefixService.myform.get("PrefixID").value) {
                var m_data = {
                    prefixMasterInsert:
                    {
                        ...
                        addedBy: 1,
                        IsActive: Boolean(JSON.parse(this._PrefixService.myform.get("IsDeleted").value)),
                    },
                };
                this._PrefixService.insertPrefixMaster(m_data).subscribe((data) => {
                    this.msg = data;
                    if (data) {
                        Swal.fire("Saved !", "Record saved Successfully !", "success").then((result) => {
                            if (result.isConfirmed) {
                                this.getPrefixMasterList();
                            }
                        });
                    } else {
                        Swal.fire("Error !", "Prefix not saved", "error");
                    }
                    this.getPrefixMasterList();
                });
            } else {
                var m_dataUpdate = {
                    prefixMasterUpdate: {
                        prefixID: this._PrefixService.myform.get("PrefixID").value,
                        prefixName: this._PrefixService.myform.get("PrefixName").value.trim(),
                        sexID: this._PrefixService.myform.get("SexID").value,
                        IsActive: Boolean(JSON.parse(this._PrefixService.myform.get("IsDeleted").value))
                        //  updatedBy: 1,
